	...
	for j in range(NUM_IMAGES):
		...
		while ( cursor_final.hasNext()):
				...
				if (cursor_final.get().get() > THRESHOLD):
						...
						cursor_output.get().set(-1.0)
				k += 1
		ij.scifio().datasetIO().save(ij.dataset().create(output),"%s/boosting_%d_imagenum_%d.jpg" % (data_dir, i, j))
#		if (i == NUM_BOOSTING - 1):
#			ui.show(output)
	print("iteration!")
for i in range(NUM_IMAGES):
	ij.scifio().datasetIO().save(ij.dataset().create(final_images[i]),"%s/final_%d.jpg" % (data_dir, i))
